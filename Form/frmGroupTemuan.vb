Public Class frmGroupTemuan
    ...
    Private Sub cmdHapus_ClickEvent(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdHapus.Click
        ...
        SQL = "DELETE FROM GROUPTEMUAN WHERE IDGROUP='" & txtIDGroup.Text & "'"
        ExecuteQuery(SQL)
        Kosong()
        txtStatus(True)
        CommandStatus(True)
    ...
    Private Sub cmdSimpan_ClickEvent(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdSimpan.Click
        If Status = "tambah" Then
            SQL = "INSERT INTO GROUPTEMUAN VALUES ('" & txtIDGroup.Text & "','" & txtUraian.Text & "')"
        ElseIf Status = "edit" Then
            SQL = "UPDATE GROUPTEMUAN SET URAIAN='" & txtUraian.Text & "' WHERE IDGROUP='" & txtIDGroup.Text & "'"
        End If
    ...
    Private Sub frmGroupTemuan_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        Fresh()
    End Sub

    Private Sub frmGroupTemuan_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Fresh()
        txtStatus(True)
        CommandStatus(True)
    End Sub
    ...
    Sub Fresh()
        SQL = "SELECT * FROM GROUPTEMUAN ORDER BY IDGROUP"
        ExportToListView(lvGroupTemuan, SQL, False)
        SQL = "SELECT IDGROUP FROM GROUPTEMUAN"
        IsiDataCombo((frmTemuan.cboGroup), SQL)
    End Sub

    Function Tampil() As Boolean
        If lvGroupTemuan.Items.Count = 0 Then
            Tampil = False
            Exit Function
        End If
        txtIDGroup.Text = lvGroupTemuan.FocusedItem.Text
        txtUraian.Text = lvGroupTemuan.FocusedItem.SubItems(1).Text
        Tampil = True
    End Function
    ...
    Private Sub lvGroupTemuan_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lvGroupTemuan.SelectedIndexChanged
