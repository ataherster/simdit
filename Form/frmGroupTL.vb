Public Class frmGroupTL
    ...
    Private Sub cmdHapus_ClickEvent(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdHapus.Click
        ...
        SQL = "DELETE FROM GROUPTINDAKLANJUT WHERE IDGROUP='" & txtIDGroup.Text & "'"
        ExecuteQuery(SQL)
        Kosong()
        txtStatus(True)
        CommandStatus(True)
    ...
    Private Sub cmdSimpan_ClickEvent(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdSimpan.Click
        If Status = "tambah" Then
            SQL = "INSERT INTO GROUPTINDAKLANJUT VALUES ('" & txtIDGroup.Text & "','" & txtUraian.Text & "')"
        ElseIf Status = "edit" Then
            SQL = "UPDATE GROUPTINDAKLANJUT SET URAIAN='" & txtUraian.Text & "' WHERE IDGROUP='" & txtIDGroup.Text & "'"
        End If
    ...
    Private Sub frmGroupTINDAKLANJUT_Activated(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Activated
        Fresh()
    End Sub

    Private Sub frmGroupTINDAKLANJUT_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Fresh()
        txtStatus(True)
        CommandStatus(True)
    End Sub
    ...
    Sub Fresh()
        SQL = "SELECT * FROM GROUPTINDAKLANJUT ORDER BY IDGROUP"
        ExportToListView(lvGroupTindakLanjut, SQL, False)
        SQL = "SELECT IDGROUP FROM GROUPTINDAKLANJUT"
        IsiDataCombo((frmTindakLanjut.cboGroup), SQL)
    End Sub

    Function Tampil() As Boolean
        If lvGroupTindakLanjut.Items.Count = 0 Then
            Tampil = False
            Exit Function
        End If
        txtIDGroup.Text = lvGroupTindakLanjut.FocusedItem.Text
        txtUraian.Text = lvGroupTindakLanjut.FocusedItem.SubItems(1).Text
        Tampil = True
    End Function
    ...
    Private Sub lvGroupTINDAKLANJUT_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lvGroupTindakLanjut.SelectedIndexChanged
